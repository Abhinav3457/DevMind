...
export function DocGeneratorPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-100">Documentation Generator</h1>
        <p className="mt-1 text-sm text-gray-500">Generate professional project documentation using AI</p>
      </div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {docTypes.map(dt => (
                <button key={dt.value} onClick={() => setDocType(dt.value)}
                  className={'flex items-center gap-2 rounded-lg border px-3 py-2.5 text-xs font-medium transition-all ' + (docType === dt.value ? 'border-primary-500/50 bg-primary-500/10 text-primary-400' : 'border-surface-700 bg-surface-800/50 text-gray-500 hover:border-surface-600 hover:text-gray-300')}
                ><dt.icon className="h-3.5 w-3.5" />{dt.label}</button>
              ))}
            </div>
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-300">Project Context</label>
            <textarea
              ...
              className="h-[300px] w-full resize-none rounded-xl border border-surface-800 bg-surface-900/50 p-4 text-sm text-gray-300 placeholder-gray-700 focus:border-primary-500/50 focus:outline-none"
            />
          </div>
          ...
          <div className="mb-3 flex items-center justify-between">
            ...
            {documentation && (
              <button onClick={handleDownload}
                className="flex items-center gap-1.5 rounded-lg border border-surface-700 bg-surface-800/50 px-3 py-1.5 text-xs text-gray-400 transition-all hover:text-gray-200"
          ...
          <div className="h-[400px] overflow-y-auto rounded-xl border border-surface-800 bg-surface-900/50 p-4 backdrop-blur-sm">
            {documentation ? (
              ...
              <div className="flex h-full flex-col items-center justify-center text-center">
                ...
                <p className="mt-1 text-xs text-gray-600">Select a document type and describe your project</p>
              </div>
            )}
          </div>
        </div>
