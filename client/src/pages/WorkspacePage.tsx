  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-surface-100">Workspaces</h1>
          <p className="mt-1 text-sm text-surface-400">Manage your development workspaces</p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {workspaces.filter(w => !search || w.name.toLowerCase().includes(search.toLowerCase())).map((ws, i) => (
            <motion.div
              ...
              className="group cursor-pointer rounded-xl border border-surface-700 bg-surface-900/50 p-5 backdrop-blur-sm transition-all hover:border-primary-500/30 hover:bg-surface-800/50"
            ...
            >
              ...
              <h3 className="mt-3 font-medium text-surface-200">{ws.name}</h3>
              {ws.description && <p className="mt-1 text-xs text-surface-400 line-clamp-2">{ws.description}</p>}
